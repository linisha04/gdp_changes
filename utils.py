...
1. **SQL Query Generation:**
   - Generate only `SELECT` statements (No `UPDATE`, `DELETE`, or `INSERT` or `DROP` or `TRUNCATE` , `CREATE`).
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=dedent(f"""you are tasked with identifying the file which contains required data, based on the query: {query}.
                    ...
                    2. consumer_price_index_cpi_for_agricultural_and_rural_labourers: this table covers data for year 2024. it should be used only when "agriculture labour" or "rural labour" is mentioned.
                    ...
                    7. none_of_these: for any queries which are unrelated to inflation. for example, queries regarding gdp, iip, msme would fall under the "none" category. queries regarding the general state of the economy, government policies, and upcoming challenges also fall under the none_of_these cateogy.
...
When setting conditions for column values, always refer to the following list for cpi_inflation_data table. This list contains the exact string matches to ensure SQL query conditions are accurate. By following this, you can avoid any mistakes while generating SQL queries:
    ...
    6. 'group_name': Choose one or more from the following list: ["Food and Beverages","Pan, Tobacco and Intoxicants","Housing","Clothing and Footwear","Consumer Food Price","Miscellaneous","General"]. Note that "General" is a special key for aggregate queries.
...
1.Always keep the state as "All India" if not specified in the question.
2.Always keep the sector as "Combined" if not specified in the question.
3.Always keep the group_name as "General" if not specified in the question.
4.Always keep the sub_group_name as "*" if not specified in the question.
5.Never provide information about the urban and rural sector if not specified in the question.
...
- For group_name="Food and Beverages" , understand what user is asking for:
  ...
  - There exists sub-group names like ""Cereals and Products" ,"Meat and Fish","Egg","Milk and Products","Oils and Fats","Fruits","Vegetables","Pulses and Products","Sugar and Confectionery","Spices","Prepared Meals, Snacks, Sweets etc.","Non-alcoholic Beverages".
...
Note:Use exact string matches for the quries generation.

state =>  ["rajasthan", "haryana", "bihar", "meghalaya", "jammukashmir", "maharashtra", "madhyapradesh", "assam", "andhrapradesh", "himachalpradesh", "allindia", "punjab", "manipur", "gujarat", "odisha", "tripura", "karnataka", "jammu&kashmir", "kerala", "westbengal", "uttarpradesh", "tamilnadu"]
year => [2024]
month =>["June" , "July"]
category =>["Food","General"]
index_value => Consumer Price Index value for the given state, year, month, and category
labour_type	=> ["rural","agricultural"]
                    """,
                   "city_wise_housing_price_indices": """
                        SOURCE="India Budget Ecconomic Survey"
This table contains the City-wise Housing Price Indices in India. The data is categorized by city, index_value , year , quarter.
Note 1: Use exact string matches for the queries generation.
Note 2: If the user query does not specify which quarter, then you should give the all the quaters information .
Note 3: If the user query does not specify a which year information is asked , then you should give the latest available data as this is the latest year data available in the database for hpi index.
Note 4: If the query is about a specific state in India, ensure you look for ALL THE CITIES in that particular state.
city => ["Meerut", "Kalyan Dombivali", "Panvel", "Jaipur", "Vasai Virar", "Patna", "Navi Mumbai", "Kochi", "Noida", 
...
year =>  [2024, 2013, 2021, 2020, 2015, 2023, 2022, 2014, 2017, 2019, 2016, 2018]
quarter => ["Q1", "Q2", "Q3", "Q4"]
                   """,
                   "whole_sale_price_index_WPI_financial_year_wise": """
                        SOURCE="India Budget Ecconomic Survey"
This table contains the Wholesale Price Index (WPI) for commodities in India Financial year wise. The data is categorized by  year, commodity_name, commodity_code ,commodity_weight ,index_value.

Note 1:Use exact string matches for the quries generation.
NOTE 2: There are 859 unique commodities in the database, so if the user query does not specify a specific commodity name, then you should give "all commodities" data for the lastest year present in the database.
Note 3: If query asks wpi for a commodity then , find out all the unique commodity_name in the database and use accurate string for filtering conditions. (THIS IS MOST IMPORTANT STEP)
Note 4: Always provide exact index_value as mentioned in the database

year=>  [2013, 2021, 2020, 2015, 2023, 2022, 2014, 2017, 2019, 2016, 2012, 2018]
commoodity_code => this is unique code for each commodity.
commodity_name =>["primary articles","food articles","food grains (cereals+pulses)","cereals","paddy","wheat","jowar","bajra" ,"maize" ,"barley" ,"ragi","pulses","gram",
                  "arhar","moong","masur","urad","peas/chawali","rajma","fruits & vegetables","vegetables","potato" , "all commodities",.......etc etc]
commodity_weight => this is the weight of the commodity in the index.
                   """,
                   "cpi_worker_data": """
                        SOURCE="INDIA BUDGET;economic survey report"

This table contains the Consumer Price Index (CPI) for Workers in India. The data is categorized by  year, time_period, worker_type ,base_year ,worker_region,index_value.

Note 1:Use exact string matches for the quries generation.
Note 2 : In worker_type column we have four types of strings. 
         a."CPI-RL" means Consumer Price Index for Rural Labourers.
         b."CPI-AL" means Consumer Price Index for Agricultural Labourers.
         c."CPI-NS" means Consumer Price Index for based on new series or updated base year
         d."CPI-IW" means Consumer Price Index for Factory/Industrial Workers.
Note 3: Always provide exact index_value as mentioned in the database

year => [2013, 2021, 2020, 2015, 2011, 2023, 2022, 2014, 2017, 2019, 2016, 2012, 2018]
time_period => ["Last Month of" ,"Average of Months"], Here "Last Month of" means CPI index value for december of the given year and "Average of Months" means CPI index value for the average of all months of the given year.
worker_type => ["CPI-RL","CPI-AL","CPI-NS","CPI-IW"] 
base_year => [2001,2012,1986,2001,2010,2016] , consider only 2012 as a base year until explicilty asked.
worker_region => ["Food" , "Rural" ,"General" ,"Urban" ,"Combined" ,"Non Food"] , this worker_region column is used to categorize the CPI data based on different worker work category.
                   """,
                   "whole_sale_price_index_WPI_calendar_wise": """
                        SOURCE="India Budget Ecconomic Survey"
This table contains the Wholesale Price Index (WPI) for commodities in India Calendar year wise. The data is categorized by  year, month, commodity_name, commodity_code ,commodity_weight ,index_value.
...
def task_descriptions_by_file_GDP(selected_file):
    selected_file = selected_file.strip()
    description = {"annual_estimate_gdp_crore":"""
DATA SOURCE => MoSPI
...
Provide all the fields values in the json
